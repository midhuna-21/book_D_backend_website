...
const sellBookValidation = (bookSelldata: Books) => {
    ...
    if (!bookSelldata.address?.street) return "Street is required";
    if (!bookSelldata.address?.city) return "City is required";
    if (!bookSelldata.address?.district) return "District is required";
    if (!bookSelldata.address?.state) return "State is required";
    if (!bookSelldata.address?.pincode) return "Pincode is required";
    if (!bookSelldata.quantity) return "Quantity is required";
    if (!bookSelldata.maxDistance) return "Maximum distance is required";
    if (!bookSelldata.latitude) return "Location not getting";
    if (!bookSelldata.longitude) return "Location not getting";
