...
const rentBookValidation = (data: Books) => {
    ...
    if (!data.extraFee === undefined) return "Extra fee is required.";
    if (!data.address?.street) return "Street is required";
    if (!data.address?.city) return "City is required";
    if (!data.address?.district) return "District is required";
    if (!data.address?.state) return "State is required";
    if (!data.address?.pincode) return "Pincode is required";
    if (!data.quantity) return "Quantity is required";
    if (!data.maxDistance) return "Maximum distance is required";
    if (!data.maxDays) return "Maximum days is required";
    if (!data.minDays) return "Minimum days is required";
