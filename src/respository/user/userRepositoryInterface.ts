...
export interface IUserRepository {
   ...
   findUserById(lenderId: string): Promise<IUser | null>;
   findUpdateUser( userId: string,filteredUser: User): Promise<IUser | null>;
   findActiveUsers(): Promise<IUser[]>;
   findUpdateProfileImage( userId: string,imageUrl: string): Promise<IUser | null>;
   findSaveToken(userId: string,resetToken: string,resetTokenExpiration: number): Promise<IUser | null>;
}
